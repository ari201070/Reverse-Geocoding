...
export default async (req, res) => {
  ...
  // Vision may use a separate Google Cloud API key; accept either variable.
  const apiKey = process.env.GOOGLE_CLOUD_API_KEY || process.env.GOOGLE_MAPS_API_KEY;
  if (!apiKey) {
    return res.status(500).json({ error: 'Vision API Key not configured (set GOOGLE_CLOUD_API_KEY)' });
  }
