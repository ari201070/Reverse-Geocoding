...
- **Idempotencia de Carga**: El script de Google Maps **nunca** debe cargarse cuando `DISABLE_PAID_GOOGLE_APIS=true` está activo. En su lugar, Leaflet se carga desde CDN y se inicializa de manera idempotente usando objetos globales de Leaflet (`L`).
- **Validación de Ámbito**: No usar variables que dependan del contexto de una función hermana sin asegurar su retorno.

## 🛠️ Implementación Técnica

- **Backend (`find-poi.js`)**: Recibe `keywords`. Filtra nombres genéricos mediante `isGenericName`. Si las APIs pagadas están deshabilitadas, omite el nivel de Google Places y utiliza la cascada local (`Local Radius DB`) y OpenCage de respaldo.
- **Frontend (`app.js`)**: Implementa `isGenericName` localmente. Si la configuración de la API indica que las APIs de pago de Google están deshabilitadas, inicializa de manera obligatoria **Leaflet** con mapas base de **OpenStreetMap** en lugar de Google Maps.
