...
4.  **Consulta Obligatoria al Arquitecto (`consult_architect_llm`)**: Es un **proceso fijo y mandatorio** del flujo de trabajo realizar una consulta al Arquitecto de Reverse Geocoding mediante el comando/MCP `consult_architect_llm` antes de redactar cualquier `implementation_plan.md` o abordar lógicas complejas, y mantener total alineación con la carpeta `REVERSE_GEOCODING` de NotebookLM (sincronizada localmente en `MASTER_PROMPTS_V3_GIS_REVERSE_GEOCODING.md` y `NOTEBOOKLM_CONSOLIDATED_NOTES.md`).
