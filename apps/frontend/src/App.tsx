...
export default function App() {
  ...
  const handleAnalyzeFile = async (file: DriveFileItem) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errCode === 'LOCAL_SERVICE_UNAVAILABLE') {
      ...
      if (err.message === 'LOCAL_SERVICE_UNAVAILABLE') {
        showToast('No se pudo conectar con el servidor local de escaneo (http://localhost:8000). Asegúrate de que Uvicorn esté iniciado.', 'error');
      } else {
  ...
  const handleAnalyzeLocalFile = async (name: string, mimeType: string, base64Data: string, localId: string, exifData?: any) => {
    ...
    try {
      const response = await fetch('/api/analyze-local', {
        ...
        body: JSON.stringify({
          ...
          mimeType,
          base64Data,
          exifData // Send EXIF data to backend
        })
      });
