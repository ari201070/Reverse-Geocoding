...
export async function generateContentWithRetry(options: any, maxRetries = 3, initialDelayMs = 2500): Promise<any> {
  ...
  const MAX_TOTAL_MS = 60_000; // Safety: abort after 60s total regardless

  while (true) {
    // Safety break: prevent infinite loop if something unexpected happens
    if (Date.now() - startTime > MAX_TOTAL_MS) {
      throw new Error(`Gemini request aborted after ${MAX_TOTAL_MS / 1000}s total timeout`);
    }
