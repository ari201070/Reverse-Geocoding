...
export default function DriveScanner({ 
  ...
  // Tabs — default to local since no Google auth
  const [activeTab, setActiveTab] = useState<'drive' | 'photos' | 'local'>('local');
  ...
  const addFilesToQueue = async (files: File[], customNames?: { [fileName: string]: string }) => {
    ...
    for (const f of files) {
        ...
        try {
          ...
          if (output) {
            exifData = {
              ...
              date: output.DateTimeOriginal ? new Date(output.DateTimeOriginal).toISOString() : undefined
            };
          }
        } catch (err) {
          console.warn('Failed to parse EXIF for', f.name, err);
        }
      }
      ...
      reader.onload = () => {
        ...
        setLocalFiles(prev => prev.map(it => it.id === localId ? { ...it, base64Data: base64 } : it));
      };
  ...
  const handleAnalyzeLocalSingle = async (lf: LocalFileItem) => {
    ...
    try {
      await onAnalyzeLocalFile(lf.name, lf.mimeType, lf.base64Data, lf.id, lf.exifData);
