...
function getCountry(photo) {
  if (photo.country && photo.country.trim()) return photo.country.trim();
  if (photo.city) {
...
function main() {
  ...
  // Enrich photos with best date and country
  ...
  }).filter(p => p.bestDate && p.country);
  
  console.log(`After filtering: ${enriched.length} photos with date + country`);
