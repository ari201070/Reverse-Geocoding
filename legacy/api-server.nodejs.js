    ...
    if (pathname && pathname.startsWith('/api/')) {
        ...
        if (fs.existsSync(filePath)) {
            try {
                ...
                // 1. Leer el body INMEDIATAMENTE para evitar perder chunks (race condition de streams de Node.js)
                let chunks = [];
                ...
                const fullImportUrl = `${fileUrl}?update=${mtime}`;
                console.log('[API-SERVER DEBUG] importing:', fullImportUrl);
                const module = await import(fullImportUrl);
