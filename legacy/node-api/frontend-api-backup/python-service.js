...
/**
 * Reconcilia texto OCR contra la base de datos completa de nombres conocidos usando RapidFuzz en Python.
 * @param {string} ocrText - Fragmento detectado por OCR.
 * @returns {{ place_id: string, name: string, score: number, match: boolean }}
 */
async function reconcileOcrDb(ocrText) {
  const res = await fetch(`${PYTHON_SERVICE_URL}/reconcile-ocr-db`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ocr_text: sanitizeString(ocrText),
  ...
  if (!res.ok) throw new Error(`Reconciliation OCR DB service error: ${res.status}`);
  return res.json();
}

export { checkHealth, analyzeExif, reconcileName, calculateConsensus, reconcileOcrDb };
